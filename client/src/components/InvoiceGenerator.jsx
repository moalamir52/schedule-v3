    ...
    <>
      <style>
        {`
          @media print {
            .letterhead-page {
              background-image: url(${letterheadImage}) !important;
...
const InvoiceGenerator = ({ clientData, onClose, onInvoiceCreated, existingRef, existingPaidStatus, bankConfig: propBankConfig, vatRate: propVatRate }) => {
  ...
  const handleExportPDF = () => {
    ...
    // NATIVE DOM ISOLATION: The Ultimate PDF Size Optimizer
    const root = document.getElementById('root');
    const letterhead = document.querySelector('.letterhead-page');
    
    // 1. Create a pure empty container for printing
    const printContainer = document.createElement('div');
    printContainer.id = 'pure-print-container';
    
    // 2. Clone the invoice into the new container
    printContainer.appendChild(letterhead.cloneNode(true));
    document.body.appendChild(printContainer);
    
    // 3. Hide the entire React App (This prevents Chrome from processing 600KB+ of hidden invoices)
    ...
    // 6. Restore original state immediately afterwards
    setTimeout(() => {
  ...
  React.useEffect(() => {
    if ((existingRef || clientData.existingRef)) {
      ...
      // Just prepare the preview number, don't auto-create
  ...
  const handleRefConfirm = async () => {
    ...
    try {
      // Generate new ref number if not already set
      ...
      if (responseData.success) {
        const invoiceRef = responseData.invoice?.invoiceId || finalRefNumber || 'INVOICE-CREATED';
