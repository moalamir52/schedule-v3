        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <div 
            onClick={() => {
              setSearchTerm('');
              setStatusFilter('All');
              // Filter for subscription invoices (not ONE_TIME)
              const subscriptionInvoices = invoices.filter(inv => inv.CustomerID && inv.CustomerID !== 'ONE_TIME');
              if (subscriptionInvoices.length > 0) {
                setSearchTerm(subscriptionInvoices[0].CustomerID.substring(0, 4)); // Search by CUST prefix
              }
            }}
              ...
              <div>
                ...
                <div style={{ maxHeight: '400px', overflow: 'auto', border: '1px solid #ddd', borderRadius: '8px' }}>
                  {(availableClients || []).filter(client => {
                    // هنا نعتمد فقط على القائمة المتاحة من السيرفر + المنطق العام أعلى الصفحة
                    // لا نعيد فحص الفواتير مرة أخرى حتى لا نخفي عملاء متاحين بالفعل
              ...
              <div style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: '5px 0', fontSize: '16px' }}>
                      {selectedClientDetails.DueDate
                        ? (selectedClientDetails.DueDate.includes('/')
                          ? selectedClientDetails.DueDate
                          : new Date(selectedClientDetails.DueDate).toLocaleDateString('en-GB'))
                        : '-'}
                    </p>
                  </div>
                  <div>
                    ...
                    <div key={index} style={{
                      ...
                      <div style={{
                        ...
                        <h5 style={{
                          ...
                          {duplicate.type === 'DUPLICATE_REF' ? '🔴 Duplicate REF Number' : duplicate.type === 'DUPLICATE_SERVICE_DATE' ? '🔴 Duplicate Service Date' : '🟡 Multiple Monthly Invoices'}
                        </h5>
                        <span style={{
                      ...
                      <div style={{ marginBottom: '10px', fontSize: '14px' }}>
                        {duplicate.type === 'DUPLICATE_REF' ? (
                          <><strong>REF:</strong> {duplicate.ref || 'N/A'} ({duplicate.count || 0} times)</>
                        ) : duplicate.type === 'DUPLICATE_SERVICE_DATE' ? (
                          <><strong>Customer:</strong> {duplicate.customerID || 'N/A'} - Same Service Date: {duplicate.serviceDate || 'N/A'} ({duplicate.count || 0} invoices)</>
                        ) : (
                          <><strong>Customer:</strong> {duplicate.customerID || duplicate.customerId || 'N/A'} in {duplicate.month || 'N/A'} ({duplicate.count || 0} invoices)</>
                        )}
                      </div>
                          ...
                          {(duplicate.invoices || []).map((invoice, idx) => (
                            <div key={idx} style={{
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <input
                                  type="checkbox"
                                  checked={selectedDuplicates.includes(invoice.InvoiceID)}
                                  onChange={(e) => {
                                    if (e.target.checked) {
                                      setSelectedDuplicates([...selectedDuplicates, invoice.InvoiceID]);
                                    } else {
                                      setSelectedDuplicates(selectedDuplicates.filter(id => id !== invoice.InvoiceID));
                                    }
                                  }}
                                  style={{ cursor: 'pointer' }}
                                />
                                <span>
                                  {invoice.InvoiceID || 'N/A'} - {invoice.CustomerName || 'N/A'} ({invoice.Villa || 'N/A'}) - AED {invoice.TotalAmount || '0'}
                                  <br/>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '15px', marginTop: '20px' }}>
              {selectedDuplicates.length > 0 && (
                <>
                  <button
                    onClick={async () => {
                      if (window.confirm(`Are you sure you want to delete ${selectedDuplicates.length} selected duplicate invoices?\n\nThis action cannot be undone.`)) {
                    ...
                    style={{
                      backgroundColor: deletingDuplicates ? '#6c757d' : '#dc3545',
                      color: 'white',
                      padding: '10px 20px',
                      border: 'none',
                      borderRadius: '6px',
                  ...
                  >
                    {deletingDuplicates ? '⏳ Deleting...' : `🗑️ Delete Selected (${selectedDuplicates.length})`}
                  </button>
                  <button
